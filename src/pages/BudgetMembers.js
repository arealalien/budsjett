...
export default function BudgetMembers() {
    ...
    return (
        <main className="dashboard">
            <header style={{ marginBottom: 20, display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                <h1 className="text-xl font-semibold">Members</h1>
                <span className="text-slate-600">Your role: <strong>{role}</strong></span>
...
function InviteInline({ slug }) {
    ...
    return (
        <form onSubmit={submit} style={{ display:'inline-flex', gap:8, alignItems:'center' }}>
            ...
            {msg && (
                <span style={{ fontSize:12, color: msg.includes('sent') ? 'green':'crimson' }}>
                    {msg}
                </span>
            )}
        </form>
