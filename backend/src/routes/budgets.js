...
router.get('/:slug/purchases', async (req, res, next) => {
    try {
        ...
        const shaped = items.map(p => ({
            ...
            paidBy: {
                id: p.paidBy?.id,
                username: p.paidBy?.username ?? null,
                displayName: p.paidBy?.displayName ?? null,
                avatarUrl: p.paidBy?.avatarUrl ?? null,
                avatarStorageKey: p.paidBy?.avatarStorageKey ?? null,
                avatarUpdatedAt: p.paidBy?.avatarUpdatedAt ?? null,
